...
export const cooperationSingleMatch = async (
    user: User,
    matchKey: string,
    team: number
): Promise<number> => {
    try {
        ...
        const arrOfDifferences: Array<number> = [];
        if (
            teamPositionSuffix === "0" ||
            teamPositionSuffix === "1" ||
            teamPositionSuffix === "2"
        ) {
            //red alliance
            for (let i = 0; i < 3; i++) {
                const teamNumber = allTeamsRows[i].teamNumber;
                if (i.toString() != teamPositionSuffix) {
                    const currDifference = await getDifferenceOneTeam(
                        user,
                        matchKey,
                        teamNumber
                    );
        ...
        return (
            arrOfDifferences.reduce((sum, curr) => sum + curr, 0) /
            arrOfDifferences.length
        );
    } catch (error) {
        console.error(error);
        throw error;
...
async function getDifferenceOneTeam(
    user: User,
    matchKey: string,
    teamNumber: number
) {
    const averageTotalTeamPoints = (
        await arrayAndAverageTeam(user, "totalpoints", teamNumber)
    ).average;
    const teamPointsInThisMatch = await await singleMatchEventsAverage(
        user,
        true,
        matchKey,
        teamNumber,
