...
export const arrayAndAverageTeam = async (
    user: User,
    metric: string,
    team: number
): Promise<{
    average: number;
    timeLine: Array<{ match: string; dataPoint: number }>;
}> => {
    try {
        const params = z
            .object({
        ...
        const groupedByTournament = matchKeys.reduce<Record<string, Match[]>>(
            (acc, match) => {
        ...
        if (metric === "stage") {
            return {
                average: await stagePicklistTeam(user, team),
                timeLine: null
            };
        }
        const timeLineArray = [];
        ...
        for (const tournament of tournamentGroups) {
            ...
            for (const match of tournament) {
                ...
                if (metric.includes("teleop") || metric.includes("Teleop")) {
                    const currData = singleMatchEventsAverage(
                        user,
                        metric.includes("point") || metric.includes("Point"),
                        match.key,
                        team,
                        metric,
                        teleopStart,
