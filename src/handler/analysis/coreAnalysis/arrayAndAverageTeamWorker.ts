    ...
    try {
        new Promise(async (resolve, reject) => {
            ...
            for (const team of data.teams) {
                ...
                const groupedByTournament = matchKeys.reduce<
                    Record<string, Match[]>
                >((acc, match) => {
                ...
                const tournamentGroups: Match[][] =
                    Object.values(groupedByTournament);
                if (metric === "stage") {
                    return {
                        average: await stagePicklistTeam(req, params.data.team),
                        timeLine: null
                    };
                }
                const timeLineArray = [];
                ...
                for (const tournament of tournamentGroups) {
                    ...
                    if (
                        metric.includes("teleop") ||
                        metric.includes("Teleop")
                    ) {
                        const currData = await teamAverageFastTournament(
                            req,
                            team,
                            metric.includes("point") ||
                                metric.includes("Point"),
                            metric,
                            tournament[0].tournamentKey,
                            teleopStart,
                            matchTimeEnd
                        );
                        currDatas.push(currData);
                    } else if (
                        metric.includes("auto") ||
                        metric.includes("Auto")
                    ) {
                        const currData = await teamAverageFastTournament(
                            req,
                        ...
                        const currData = teamAverageFastTournament(
                            req,
                            team,
                            metric.includes("point") ||
                                metric.includes("Point"),
                    ...
                    await Promise.all(currDatas).then(values => {
                        for (let i = 0; i < values.length; i++) {
                            if (values[i] !== null) {
                                timeLineArray.push({
                                    match: tournament[i].key,
                                    dataPoint: values[i],
                                    tournamentName: tournament[i].tournamentName
                                });
                                if (currAvg === null) {
                                    currAvg = values[i];
                                } else {
                ...
                for (let i = 0; i < tournamentAverages.length; i++) {
                    if (i === 0) {
                        ...
                        const recentTournament =
                            matchKeys[matchKeys.length - 1].tournamentKey;
                        const scoutedMatchesAtMostRecentTournament =
                            timeLineArray.filter(item =>
                                item.match.startsWith(recentTournament)
                            ).length;
                        const totalMatchesAtMostRecentTournament =
                            tournamentGroups[i].length;
                        //0.95 because thats were it asymmpotes, not where it will realsitically reach. Graph on desmos to see more info
                        const weightOnRecent =
                            0.95 *
                            (1 -
                                1 /
                                    ((multiplerBaseAnalysis *
                                        scoutedMatchesAtMostRecentTournament) /
                                        totalMatchesAtMostRecentTournament +
                                        1));
                        runningAverage =
                            runningAverage * (1 - weightOnRecent) +
                            tournamentAverages[i] * weightOnRecent;
                    } else {
                        runningAverage =
                            runningAverage * 0.2 + tournamentAverages[i] * 0.8;
                    }
                }
