...
export const teamAverageFastTournament = async (
    user: User,
    team: number,
    isPointAverage: boolean,
    metric1: string,
    tournamentKey: string,
    timeMin: number = 0,
    timeMax: number = matchTimeEnd
): Promise<number> => {
    try {
        let position = null;
        ...
        if (metric1 === "pickups") {
            ...
            return (
                counts.reduce((acc, cur) => acc + cur._count._all, 0) /
                counts.length
            );
            ...
            let eventsAverage =
                sumOfMatches.reduce((acc, item) => acc + item._sum.points, 0) /
                sumOfMatches.length;
            ...
            if (metric1 === "totalpoints" || metric1 === "teleoppoints") {
                ...
                if (totalAttemptsStage !== 0) {
                    stagePoints =
                        ((stageDataMap["ONSTAGE"] || 0) / totalAttemptsStage) *
                            3 +
                        ((stageDataMap["ONSTAGE_HARMONY"] || 0) /
                            totalAttemptsStage) *
                            5 +
                ...
                const totalAttempsHighNote = highNoteRows.reduce(
                    (total, item) => {
                ...
                if (highNotePoints !== 0) {
                    highNotePoints =
                        highNoteMap["SUCCESSFUL"] / totalAttempsHighNote;
                }
            ...
            let avg =
                groupedMatches.reduce((accumulator, current) => {
