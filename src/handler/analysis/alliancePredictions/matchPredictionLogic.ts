...
export const matchPredictionLogic = async (
    ...
    try {
        ...
        const redArr1 = (
            await arrayAndAverageTeam(user, "totalpoints", params.data.red1)
        ).timeLine.map(item => item.dataPoint);
        const redArr2 = (
            await arrayAndAverageTeam(user, "totalpoints", params.data.red2)
        ).timeLine.map(item => item.dataPoint);
        const redArr3 = (
            await arrayAndAverageTeam(user, "totalpoints", params.data.red3)
        ).timeLine.map(item => item.dataPoint);
        ...
        const blueArr1 = (
            await arrayAndAverageTeam(user, "totalpoints", params.data.blue1)
        ).timeLine.map(item => item.dataPoint);
        const blueArr2 = (
            await arrayAndAverageTeam(user, "totalpoints", params.data.blue2)
        ).timeLine.map(item => item.dataPoint);
        const blueArr3 = (
            await arrayAndAverageTeam(user, "totalpoints", params.data.blue3)
        ).timeLine.map(item => item.dataPoint);
